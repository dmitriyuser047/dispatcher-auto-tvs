/**
 * Updater module — проверка и загрузка обновлений (приложение ДЭС)
 *
 ...
 *     "url": "https://example.com/updates/Dispatcher-DES-Setup-0.1.0.exe"
 *   }
 *
...
// ─── НАСТРОЙКА ───────────────────────────────────────────────────────────────
const UPDATE_CHECK_URL = 'https://raw.githubusercontent.com/dmitriyuser047/dispatcher-auto-tvs/main/des-app/version.json';
// ─────────────────────────────────────────────────────────────────────────────
...
function downloadFile(url, onProgress, redirects = 5) {
  return new Promise((resolve, reject) => {
    ...
    const req = mod.get(url, { timeout: 0 }, res => {
      ...
      let received   = 0;
      const tmpPath  = path.join(os.tmpdir(), 'dispatcher-des-update-setup.exe');
